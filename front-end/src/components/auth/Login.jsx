...
const Login = () => {
  ...
  // Listen once on mount for session or auth changes (handles OAuth redirect)
  useEffect(() => {
    let mounted = true;
    ...
    const checkSession = async () => {
      try {
        ...
        if (session && mounted) {
          // Session exists (user returned from OAuth)
          setLoading(true);
  ...
  if (loading) {
    return (
      <div className="flex items-center justify-center min-h-screen">
        <span className="text-xl font-semibold">Logging in with Google, please wait...</span>
      </div>
    );
  }
      ...
      <div className="relative z-10 flex flex-col items-center justify-center w-full px-6 py-12">
        ...
        >
          <form onSubmit={handleSubmit} className="flex flex-col gap-4">
            ...
            <button className="w-full py-3 mt-5 bg-gradient-to-r from-[#5044E5] to-[#4d8cea] text-white rounded-full font-semibold shadow-lg hover:scale-[1.03] transition">Sign in</button>
          </form>
          <button
            type="button"
